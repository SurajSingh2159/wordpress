...
if( !function_exists( 'woo_vl_dashboard_setup' ) ) {
	...
	function woo_vl_dashboard_setup() {
		...
		if( current_user_can( $user_capability ) ) {
			if( apply_filters( 'woo_vl_news_widget', true ) ) {
				$dashboard_widget_title = __( 'Plugin News - by Visser Labs', 'woocommerce-product-importer' );
	...
	function woo_vl_news_widget() {
		...
		if( !is_wp_error( $rss ) ) {
			...
			$message = __( 'Connection failed. Please check your network settings.', 'woocommerce-product-importer' );
			$output .= '<p>' . $message . '</p>';
		}
		$output .= '</div>';
